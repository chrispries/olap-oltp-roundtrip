...
slug — e.g. `jane.doe@acme.com` → `jane_doe`, so the catalog is `lakebase_schema_jane_doe`.
...
FROM lakebase_schema_<your_user>.public.maintenance_actions
...
FROM lakebase_schema_<your_user>.public.maintenance_actions a
JOIN lakebase_schema_<your_user>.public.machines m           ON m.machine_id = a.machine_id
LEFT JOIN lakebase_schema_<your_user>.public.maintenance_tickets t ON t.ticket_id = a.ticket_id
...
print("done — drop your Postgres database schema_%s from the SQL/psql side if you want it gone too" % slug)
```
